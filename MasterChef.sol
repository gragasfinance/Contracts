...
// File: contracts\GragasToken.sol

pragma solidity 0.6.12;

// GragasToken with Governance.
contract GragasToken is BEP20('GragasFinance', 'GRAGAS') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "GRAGAS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "GRAGAS::delegateBySig: invalid nonce");
        require(now <= expiry, "GRAGAS::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "GRAGAS::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GRAGASs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "GRAGAS::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of Gragas. He can make Gragas and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GRAGAS is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GRAGASs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accGragasPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGragasPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. GRAGASs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that GRAGASs distribution occurs.
        uint256 accGragasPerShare;   // Accumulated GRAGASs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The GRAGAS TOKEN!
    GragasToken public gragas;
    // Dev address.
    address public devaddr;
    // GRAGAS tokens created per block.
    uint256 public gragasPerBlock;
    // Bonus muliplier for early gragas makers.
